...
public class FilmService : IFilmServices
{
    ...
    public async Task<FilmOutputDto> CreateAsync(FilmSaveDto filmSaveDto)
    {
        ...
        var actors = _filmRepository
            .GetActorsByNameAndSurname(filmSaveDto.Actors.Select(a => new[] { a?.Name, a?.Surname })).ToList();
        ...
        if (notFoundActors.Any())
            throw new ActorNameAndSurnameNotValidException(actors.Select(a => a.Name).ToString(),
                actors.Select(a => a.Surname).ToString()!);
    ...
    public async Task<FilmOutputDto> UpdateAsync(int id, FilmSaveDto filmSaveDto)
    {
        ...
        if (film.EarningSale != null && filmSaveDto.TotalEarning != film.EarningSale.TotalEarning &&
            filmSaveDto.TotalEarning <= 0)
            throw new CheckException(" Il TotalEarning  del film non può essere minore o uguale a 0.");
        ...
        if (CheckChangesInToActors(filmSaveDto, film))
        {
            ...
            foreach (var actorDto in filmSaveDto.Actors)
            {
                var actor = actors.FirstOrDefault(a => a.Name == actorDto?.Name && a.Surname == actorDto.Surname);
                if (actor != null)
                {
                    film.AddActor(actor);
                }
